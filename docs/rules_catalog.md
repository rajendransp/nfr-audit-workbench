...
| NFR-DOTNET-003 | Blocking wait in async-capable code | S2 | concurrency | 0.5 |
...
| NFR-DOTNET-012 | Fire-and-forget background work pattern | S2 | concurrency | 0.7 |
| NFR-DOTNET-013 | Explicit CancellationToken.None | S2 | concurrency | 0.75 |
| NFR-DOTNET-014 | Empty catch block | S2 | loading | 0.7 |
...
| NFR-DOTNET-021 | Synchronous JSON serialization in hot path (review) | S3 | performance | 0.65 |
...
| NFR-API-003 | No pagination on collection response | S3 | loading | 0.5 | review |
| NFR-API-004A | Manual JSON serialization in API path | S2 | performance | 0.8 | review |
...
| NFR-API-009 | No [ApiController] on controller class | S4 | performance | 0.8 | review |
| NFR-API-010 | Bulk endpoint without max batch constraint | S2 | loading | 0.65 | review |
| NFR-API-011 | No rate limiting middleware configured | S2 | loading | 0.6 | review |
...
| NFR-API-015 | Critical endpoint marked AllowAnonymous | S2 | authentication | 0.75 | review |
